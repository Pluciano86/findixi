export const PLANES_PRELIMINARES = [
  {
    ...
    features: ['Logo y portada', '1 categoría', 'Teléfono y dirección', 'Municipio y coordenadas', 'Horario'],
  },
  {
    slug: 'regular',
    ...
    features: ['Todo lo de Basic', 'Galería', 'Redes sociales', 'Descripción', 'Amenidades'],
  },
  {
    slug: 'plus',
    nombre: 'Findixi Plus',
    ...
    nivel: 2,
    descripcion_corta: 'Plan mensual',
    features: ['Todo lo de Regular', 'Menú (secciones/productos)', 'Happy Hours y Almuerzos'],
  },
  {
    slug: 'premium',
    ...
    nivel: 3,
    descripcion_corta: 'Plan mensual',
    features: ['Todo lo de Plus', 'Órdenes online con Clover'],
  },
];

const PLAN_SLUGS = new Map(PLANES_PRELIMINARES.map((plan) => [plan.slug, plan.nivel]));
...
function propiedadVerificadaDesdeEstado(comercio = {}) {
  ...
  const propietarioVerificado =
    typeof comercio.propietario_verificado === 'boolean' ? comercio.propietario_verificado : null;

  const hasOwnershipSignals =
    Boolean(estadoPropiedad) || Boolean(estadoVerificacion) || typeof propietarioVerificado === 'boolean';
  ...
  const verificado = estadoPropiedad === 'verificado' && (verificacionOk || propietarioVerificado === true);
...
export function resolverPlanComercio(comercio = {}) {
  ...
  const tienePlanExpl =
    (rawNivel !== undefined && rawNivel !== null && rawNivel !== '') ||
