#!/usr/bin/env python3
"""
Sincroniza export CSV de Pietix hacia Supabase.

Flujo:
1) Lee CSVs en exports/pietix/
2) Upsert de eventos (sin borrar todo)
3) Reemplaza sedes/fechas para eventos sincronizados
4) Actualiza boleterías (source=pietix)
"""
...
SOURCE_NAME = "pietix"
EXPORT_DIR = Path("exports/pietix")
EVENTOS_CSV = EXPORT_DIR / "eventos_pietix.csv"
SEDES_CSV = EXPORT_DIR / "eventos_municipios_pietix.csv"
FECHAS_CSV = EXPORT_DIR / "eventoFechas_pietix.csv"
BOLETERIAS_CSV = EXPORT_DIR / "eventos_boleterias_pietix.csv"
...
def sync() -> int:
    ...
    # Mapa de evento existente por URL (source pietix)
    ...
    # 5) Reemplazar boleterías Pietix para estos eventos
    sb.delete(
        "eventos_boleterias",
        {
            "source": f"eq.{SOURCE_NAME}",
    ...
    print(f"[sync_pietix] sedes_sin_localidad_generica: {sedes_sin_localidad_generica}")
    print(f"[sync_pietix] sedes_sin_localidad_no_pr: {sedes_sin_localidad_non_pr}")
    print(f"[sync_pietix] sedes_localidad_con_coords: {sedes_localidad_con_coords}")
