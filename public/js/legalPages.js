...
const LEGAL_CONTENT = {
  es: {
    privacy: {
      ...
      intro: 'Findixi ("nosotros") respeta tu privacidad y se compromete a proteger tu información personal.',
      sections: [
        {
...
function resolveLang() {
  const current = String(getLang?.() || localStorage.getItem('lang') || 'es').toLowerCase();
  return LEGAL_CONTENT[current] ? current : 'es';
}
...
function renderSection(section) {
  const paragraphs = (section.paragraphs || [])
    .map((paragraph) => `<p class="text-sm leading-6">${escapeHtml(paragraph)}</p>`)
    .join('');

  const bullets = section.bullets?.length
    ? `<ul class="list-disc list-inside space-y-1">${section.bullets
        .map((bullet) => `<li class="text-sm leading-6">${escapeHtml(bullet)}</li>`)
        .join('')}</ul>`
    : '';

  const emailLine = section.emailLine
    ? `<p class="text-sm leading-6">${escapeHtml(section.emailLine)} <a href="mailto:${EMAIL}" class="text-blue-600 hover:underline">${EMAIL}</a></p>`
    : '';

  return `
    <div class="space-y-2">
      <h2 class="text-lg font-semibold">${escapeHtml(section.title)}</h2>
      ${paragraphs}
      ${bullets}
      ${emailLine}
    </div>
...
function renderLegalPage() {
  ...
  const copy = LEGAL_CONTENT[lang][page] || LEGAL_CONTENT.es[page];

  const titleEl = document.getElementById('legalTitle');
  const updatedEl = document.getElementById('legalUpdated');
  const introEl = document.getElementById('legalIntro');
  ...
  if (updatedEl) updatedEl.textContent = copy.updated;
  if (introEl) introEl.textContent = copy.intro;
  if (contentEl) contentEl.innerHTML = copy.sections.map(renderSection).join('');
}

if (typeof window !== 'undefined') {
  renderLegalPage();
  window.addEventListener('lang:changed', renderLegalPage);
  window.addEventListener('storage', (event) => {
    if (event.key === 'lang') renderLegalPage();
  });
}
