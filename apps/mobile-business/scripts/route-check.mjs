...
function fail(message) {
  console.error(`[mobile-business][route-check] ${message}`);
  process.exit(1);
}

function info(message) {
  console.log(`[mobile-business][route-check] ${message}`);
}
...
const REQUIRED_ROUTES = ['/', '/login', '/pedidos', '/perfil'];
...
function main() {
  ...
  if (missingRequired.length > 0) {
    for (const route of missingRequired) {
      console.error(`[mobile-business][route-check] Missing required route file for ${route}`);
    }
  ...
  if (unresolved.length > 0) {
    for (const item of unresolved) {
      console.error(`[mobile-business][route-check] Unresolved route ${item.target} in ${item.file}`);
    }
